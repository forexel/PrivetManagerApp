...
function ClientDetailPage() {
  ...
  const profileMutation = useMutation({
    mutationFn: (payload: ClientProfileUpdate) => api.updateProfile(clientId, payload),
    onSuccess: (detail) => updateDetailCache(detail),
  })

  const passportMutation = useMutation({
    mutationFn: (payload: PassportUpsert) => api.upsertPassport(clientId, payload),
    onSuccess: (detail) => updateDetailCache(detail),
  })
  ...
  const updateDeviceMutation = useMutation({
    mutationFn: ({ deviceId, payload }: { deviceId: string; payload: DeviceUpdate }) =>
      api.updateDevice(clientId, deviceId, payload),
    onSuccess: (detail) => updateDetailCache(detail),
  })
  ...
  const deletePhotoMutation = useMutation({
    mutationFn: ({ deviceId, photoId }: { deviceId: string; photoId: string }) =>
      api.deleteDevicePhoto(clientId, deviceId, photoId),
    onSuccess: (detail) => updateDetailCache(detail),
  })

  const applyTariffMutation = useMutation({
    mutationFn: (payload: TariffCalculateRequest) => api.applyTariff(clientId, payload),
    onSuccess: (detail) => updateDetailCache(detail),
  })
  ...
  const billingMutation = useMutation({
    mutationFn: (payload: BillingNotifyRequest) => api.notifyBilling(clientId, payload),
    onSuccess: (detail) => updateDetailCache(detail),
  })
    ...
    <div className="client-detail">
      ...
      <section className="client-detail__section">
        <div className="section-header">
          <h2>Шаг 3. Устройства ({data.devices.length})</h2>
          <button type="button" onClick={() => setDeviceFormVisible((prev) => !prev)}>
            {isDeviceFormVisible ? 'Скрыть форму' : 'Добавить устройство'}
          </button>
        </div>

        {isDeviceFormVisible && (
          <form className="client-form" onSubmit={handleAddDevice}>
            <label>
              Тип устройства
              <select {...deviceForm.register('device_type')}>
            ...
            <label>
              Название
              <input type="text" {...deviceForm.register('title')} />
            </label>
            <label>
              Описание
              <textarea rows={3} {...deviceForm.register('description')} />
            </label>
            <label>
              Доплата
              <input type="number" step="100" min="0" {...deviceForm.register('extra_fee', { valueAsNumber: true })} />
            </label>
          ...
          {data.devices.map((device) => (
            <li key={device.id} className="device-card">
              <div>
                ...
                <p>Доплата: {device.extra_fee.toLocaleString('ru-RU')} ₽</p>
                {device.photos.length > 0 && (
                  <div className="device-photos">
                    {device.photos.map((photo) => (
                      <div key={photo.id} className="device-photo">
                        <a href={photo.file_url} target="_blank" rel="noreferrer">
                          Фото
                        </a>
                        <span className="device-photo__key">{photo.file_key}</span>
                        <button
                          type="button"
                          onClick={() => deletePhotoMutation.mutate({ deviceId: device.id, photoId: photo.id })}
                ...
                <label className="device-photo-upload">
                  Загрузить фото
                  <input
                    type="file"
                    accept="image/*"
                    onChange={(event) => handleUploadPhoto(device.id, event.target.files?.[0] ?? null)}
                ...
                <button
                  ...
                  onClick={() =>
                    updateDeviceMutation.mutate({
                      deviceId: device.id,
                      payload: { extra_fee: device.extra_fee + 100 },
                    })
                  }
                >
                  +100 ₽
                </button>
                <button type="button" onClick={() => deleteDeviceMutation.mutate(device.id)}>
                  Удалить
                </button>
              </div>
            </li>
